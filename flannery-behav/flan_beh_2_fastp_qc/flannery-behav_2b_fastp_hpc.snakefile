...
To perform QC on metagenomics data using fastp 0.23.2

Run from Deep Thought folder - /scratch/user/lidd0026/flannery-behav/flan_beh_2_fastp_qc
...
READDIR = '/scratch/user/lidd0026/flannery-behav/flan_beh_1_meta_raw'

# directory for QC outputs
OUTDIR = '/scratch/user/lidd0026/flannery-behav/flan_beh_2_fastp_qc'


# Note that this example requires an R1 file AND an R2 file
# and that each file should match *_R1* and *_R2*
SAMPLES,EXTENSIONS = glob_wildcards(os.path.join(READDIR, '{sample}_pass_1.{extensions}'))
...
rule run_fastp:
    input:
        r1 = os.path.join(READDIR, "{sample}_pass_1." + file_extension),
        r2 = os.path.join(READDIR, "{sample}_pass_2." + file_extension)
    output:
