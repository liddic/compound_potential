# # # # # # # # # # # # # # # #
# # Run Superfocus fxn annotations from reads - for Sun & Badgley 2019 post-mining forest restoration soil metagenomics samples
# # Run from Deep Thought folder - /scratch/user/lidd0026/sunbad-resto/sunbad-resto_3_superfocus_fxns
# # # # # # # # # # # # # # # #
...
READDIR = '/scratch/user/lidd0026/sunbad-resto/sunbad-resto_2_mgrast_screened'

# set working directory
workDir = '/scratch/user/lidd0026/sunbad-resto/sunbad-resto_3_superfocus_fxns'
...
table = pd.read_csv(os.path.join(READDIR,'sunbad-resto-mgp16379_metadata_csv.csv'), sep=',')

# extract info from pandas data table
samples = table["metagenome_id"]
...
for i in range(n):
    ...
    qc_read_r1 = os.path.join(READDIR, "%s/%s.299.screen.passed.fna" % (samples[i],samples[i]))
